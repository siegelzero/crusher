...
##   int_lin_eq([1,...,1], [..., c, ...], K)       -- the count constraint
##
## When b's only constraint usages are the clause and the bool2int (degree 2),
## we channel b = AND(forcing literals), which removes b from search positions
## and ties it directly to its forcing condition.
##
## Originally motivated by HRC blocking-pair indicators (MZN Challenge 2019/hrc):
## 353 bp[i,j] bools each forced by `pos[i] > j ∧ would_prefer` and summed to
## `Σ bp = num_bp`. Without this rewrite they remain free search positions,
## severing the gradient between pos and the count.
...
"""
  ...
  test "multi-positive clause: extra positives appear in AND-target's body":
    ...
    ## With two indicators summing to 1, neither is statically forced, so
    ## presolve leaves b1's domain as {0,1} and the build phase succeeds.
    let src = """
var bool: P1;
var bool: P2;
var bool: N1;
...
"""
  ...
  test "channels propagate end-to-end: HRC bp pattern shape":
    ## End-to-end: with N free, channel binding gives b = (¬P ∧ N). Setting
    ## only the inputs and propagating channels reaches the right b values.
    ## The sum constraint then forces a unique LHS to fire.
    let src = """
